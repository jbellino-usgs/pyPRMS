...
from .parbase import ParameterDict
from .dimbase import DimensionDict
...
class ParameterFile(object):
    ...
    def _read(self):
        ...
        for line in it:
            ...
            if self.__verbose and 'hru' in varname:
                print('Processing', varname)

            # Add the parameter
            try:
                if self.parameters.master_parameters is not None:
                    self.parameters.add(varname,
                                        info=self.parameters.master_parameters[
                                            varname])
                else:
                    self.parameters.add(varname)
            ...
            #     try:
            #         master_info =
            #         self.parameters.master_parameters.parameters[varname]
            #
